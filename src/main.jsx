...
const approvalModels = ['founder_approvals', 'notifications', 'audit_logs', 'workflow sync'];
...
function App() {
  ...
  if (route === '/export-os/tasks') {
    return <TaskFollowupEngine navigate={navigate} onBack={() => navigate('/export-os')} />;
  }
  ...
  if (route === '/export-os/shipments') {
    return <ShipmentTrackerPage navigate={navigate} onBack={() => navigate('/export-os')} />;
  }

  if (route.startsWith('/export-os/shipments/')) {
    const shipmentId = route.split('/').pop();
    return <ShipmentTrackerPage navigate={navigate} onBack={() => navigate('/export-os/shipments')} shipmentId={shipmentId} />;
  }
...
function ShipmentPage() {
  return <ShipmentTrackerPage onBack={() => {}} navigate={() => {}} />;
}

const shipmentFilterOptions = ['All', 'Active', 'In Transit', 'Delivered', 'Issue / Hold'];

function ShipmentTrackerPage({ navigate, onBack, shipmentId }) {
...
function CompanyMasterDataVault({ onBack }) {
  ...
  useEffect(() => {
    ...
    async function loadVaultData() {
      ...
      if (error) setServiceError(error.message || 'Unable to load some company data. Unavailable is active.');
      setLoading(false);
    }
  ...
  return (
    <section className="payment-vault-panel billing-rules-panel">
      ...
      <div className="billing-rule-list">
        <span>Up to ₹1,000: safe auto-renew allowed only for trusted infrastructure vendors with approved token and no risk flag.</span>
...
function WorkflowDependencyEngine({ navigate, onBack }) {
  const [now, setNow] = useState(() => new Date());
  const [data, setData] = useState(null);
...
function WorkflowJourneyDashboard({ navigate, onBack }) {
  const [now, setNow] = useState(() => new Date());
  const [data, setData] = useState(null);
  const [selectedWorkflow, setSelectedWorkflow] = useState(null);
...
function WorkflowDetailPage({ workflowId, navigate, onBack }) {
