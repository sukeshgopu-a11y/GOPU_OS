...
import { validateCmoCanvaBeforePublish } from "./cmoChatGPTImageWorkflow.mjs";
...
async function publishInstagram(row) {
  ...
  if (!imageUrl) return { ok: false, status: "missing_image", error: "Instagram publishing blocked: A generated image is required." };
...
async function publishFacebook(row) {
  ...
  const imageUrl = row.image_url || row.poster_url;
  if (!imageUrl) return { ok: false, status: "missing_image", error: "Facebook publishing blocked: A generated image is required." };
...
async function executeOnePublish(client, row, { dryRun = false } = {}) {
  ...
  const imageValidation = validateCmoCanvaBeforePublish(row);
  if (!imageValidation.ok) {
    const failed = await updateContent(client, row.id, {
      publish_status: "failed",
      last_publish_error: imageValidation.error,
      metadata: {
        ...metadata(row),
        current_step: 7,
        workflow_stage: "publishing",
        step7_status: "blocked_missing_image",
        image_publish_guard: imageValidation
      }
    });
    await writeAudit(client, failed, "cmo_publish_blocked_missing_image", imageValidation.error, { publish_status: "failed", risk_level: "High", metadata: { provider_status: imageValidation.status } });
    await sendPublishingSlackNotification({ type: "Publish Blocked", priority: "URGENT", row: failed, status: "blocked_missing_image", message: imageValidation.error });
    return { ok: false, status: "blocked_missing_image", content_history: failed, message: imageValidation.error };
  }
