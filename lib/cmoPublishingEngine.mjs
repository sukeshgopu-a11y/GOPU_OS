...
import { ensureLinkedInPostRules, validateLinkedInPublishText } from "./cmoLinkedInRules.mjs";
import { validateCmoCanvaBeforePublish } from "./cmoCanvaWorkflow.mjs";
import { cleanSlackText } from "./slackTextClean.js";
import { getMetaConfig, publishFacebookPagePost, publishInstagramImagePost } from "./metaIntegration.mjs";
...
function safeProviderError(error) {
  return safeErrorMessage(error).replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [redacted]");
}
...
function metaConfig() {
  ...
  return {
    ...
    instagramId: config.instagramBusinessAccountId,
    facebookPageId: config.pageId,
    instagramConfigured: config.instagramMissing.length === 0,
    facebookConfigured: config.facebookMissing.length === 0,
    missingInstagram: config.instagramMissing,
    missingFacebook: config.facebookMissing
  };
}
...
async function generatePostImage(imagePrompt, platform = "Instagram") {
  ...
  if (!apiKey) {
    console.warn("[cmo-publishing] OpenAI key not set in CTO provider vault or OPENAI_API_KEY; skipping image generation");
    return null;
  }
  if (!imagePrompt?.trim()) return null;

  const size = ["LinkedIn", "YouTube"].includes(platform) ? "1792x1024" : "1024x1024";
  try {
...
async function publishInstagram(row) {
  ...
  const imageUrl = row.image_url || row.poster_url;
  if (!imageUrl) return { ok: false, status: "missing_canva_image", error: "Instagram publishing blocked: Canva export image is required." };
  return publishInstagramImagePost({ caption, imageUrl });
}
...
async function publishFacebook(row) {
  ...
  const imageUrl = row.image_url || row.poster_url;
  if (!imageUrl) return { ok: false, status: "missing_canva_image", error: "Facebook publishing blocked: Canva export image is required." };
  return publishFacebookPagePost({ message, imageUrl });
}
...
function linkedinConfig() {
  ...
  const version = env("LINKEDIN_API_VERSION") || "202605";
  return {
...
function linkedinCompanyPageAvailable(config = linkedinConfig()) {
  return Boolean(env("LINKEDIN_ORGANIZATION_ID") || String(config.authorUrn || "").includes("urn:li:organization:"));
}
...
async function sendPublishingSlackNotification({ type, priority = "INFO", row, status, message }) {
  ...
  const text = [
    ...
    "",
    "*Action needed*",
    message || "Review CMO publishing in GOPU OS.",
    "",
    "*Safety*",
    "Publishing remains governed by founder approval and platform credential checks."
  ].join("\n");
  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({ text: cleanSlackText(text) })
    });
...
async function publishLinkedIn(row) {
  ...
  const prepared = ensureLinkedInPostRules(selectedCaption(row), {
    platform: "LinkedIn",
    topic: row.topic || row.campaign_name || metadata(row).topic || "",
    hashtags: row.hashtags || metadata(row).hashtags || [],
    companyPageAvailable: linkedinCompanyPageAvailable(config)
  });
  const text = prepared.text;
  if (!text.trim()) {
    return { ok: false, status: "missing_content", error: "LinkedIn post text is empty." };
  }
  const validation = validateLinkedInPublishText(text);
...
async function readLinkedInPersonalConfig(client) {
  const { data, error } = await client
    .from("platform_integrations")
    .select("metadata,status,provider,platform_key")
    .eq("platform_key", "linkedin_personal")
    .maybeSingle();

  if (error) return { ok: false, status: "config_read_failed", error: error.message };
  const rowMetadata = metadata(data);
  const token = String(rowMetadata.access_token || "").trim();
  const userId = String(rowMetadata.linkedin_user_id || rowMetadata.sub || "").trim();
  const authorUrn = userId ? `urn:li:person:${userId}` : "";
  const expiresAt = rowMetadata.expires_at || rowMetadata.token_expires_at || null;
  const expired = expiresAt ? new Date(expiresAt) <= new Date() : false;

  if (!token) return { ok: false, status: "missing_credentials", error: "LinkedIn Personal is not connected." };
  if (expired) return { ok: false, status: "token_expired", error: "LinkedIn Personal access token is expired. Reconnect LinkedIn OAuth." };
  if (!authorUrn) return { ok: false, status: "missing_author", error: "LinkedIn Personal account id is missing. Reconnect LinkedIn OAuth." };

  return {
    ok: true,
    token,
    authorUrn,
    version: env("LINKEDIN_API_VERSION") || "202605"
  };
}

async function publishLinkedInPersonal(client, row) {
  const config = await readLinkedInPersonalConfig(client);
  if (!config.ok) {
    return { ok: false, status: config.status, error: config.error };
  }

  const rowMetadata = metadata(row);
  const explicitPublishMediaUrl = rowMetadata.linkedin_personal_media_url || rowMetadata.publish_media_url || rowMetadata.media_url;
  if (explicitPublishMediaUrl) {
    return {
      ok: false,
      status: "unsupported_media",
      error: "LinkedIn Personal image publishing is not enabled yet. Text-only publishing is supported."
    };
  }

  const prepared = ensureLinkedInPostRules(selectedCaption(row), {
    platform: "LinkedIn Personal",
    topic: row.topic || row.campaign_name || rowMetadata.topic || "",
    hashtags: row.hashtags || rowMetadata.hashtags || []
  });
  const text = prepared.text;
  if (!text.trim()) {
    return { ok: false, status: "missing_content", error: "LinkedIn Personal post text is empty." };
  }
  const validation = validateLinkedInPublishText(text);
  if (!validation.ok) return validation;
  ...
  if (!response.ok) {
    return {
      ok: false,
      status: "linkedin_personal_api_failed",
      error: safeProviderError(parsed?.message || parsed?.error?.message || `LinkedIn API returned HTTP ${response.status}.`)
    };
  }
...
async function executeOnePublish(client, row, { dryRun = false } = {}) {
  ...
  if (!["LinkedIn", "LinkedIn Personal", "Instagram", "Facebook"].includes(platform)) {
    const failed = await updateContent(client, row.id, {
      publish_status: "failed",
      last_publish_error: `Unsupported platform: ${row.platform || "unknown"}. Supported: LinkedIn Personal, LinkedIn, Instagram, Facebook.`,
  ...
  const canvaValidation = validateCmoCanvaBeforePublish(row);
  if (!canvaValidation.ok) {
    const failed = await updateContent(client, row.id, {
      publish_status: "failed",
      last_publish_error: canvaValidation.error,
      metadata: {
        ...metadata(row),
        current_step: 7,
        workflow_stage: "publishing",
        step7_status: "blocked_missing_canva_design",
        canva_publish_guard: canvaValidation
      }
    });
    await writeAudit(client, failed, "cmo_publish_blocked_missing_canva", canvaValidation.error, { publish_status: "failed", risk_level: "High", metadata: { provider_status: canvaValidation.status } });
    await sendPublishingSlackNotification({ type: "Publish Blocked", priority: "URGENT", row: failed, status: "blocked_missing_canva_design", message: canvaValidation.error });
    return { ok: false, status: "blocked_missing_canva_design", content_history: failed, message: canvaValidation.error };
  }
  ...
  const publishResult = platform === "Instagram"
    ...
    : platform === "Facebook"
      ...
      : platform === "LinkedIn Personal"
        ? await publishLinkedInPersonal(client, publishRow)
  ...
  const published = await updateContent(client, reserved.id, {
    ...
    metadata: {
      ...
      linkedin_personal_post_id: platform === "LinkedIn Personal" ? publishResult.provider_post_id : undefined,
      linkedin_personal_post_urn: platform === "LinkedIn Personal" ? publishResult.provider_post_urn : undefined,
      linkedin_personal_publish_status: platform === "LinkedIn Personal" ? publishResult.status : undefined,
      published_text: publishResult.published_text || undefined
    }
  });
...
export async function runCmoPublishingEngine(options = {}) {
  ...
  const baseSelect = "id,tenant_id,run_id,platform,topic,campaign_name,caption,generated_text,final_text,final_approved_content,hashtags,image_prompt,poster_url,image_url,approval_status,publish_status,live_post_url,post_url,publish_attempt_count,last_publish_attempt_at,last_publish_error,metadata,updated_at";
  let query = client
    .from("content_history")
    .select(baseSelect)
    .eq("approval_status", "approved")
  ...
  const publishableRows = rows.filter((row) => dryRun || !isTestMode(row));
  const results = [];
