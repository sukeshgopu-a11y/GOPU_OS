...
async function publishInstagram(row) {
  ...
  const imageUrl = row.image_url || row.poster_url || await generatePostImage(row.image_prompt || row.metadata?.image_prompt, "Instagram");
...
async function publishFacebook(row) {
  ...
  const imageUrl = row.image_url || row.poster_url || await generatePostImage(row.image_prompt || row.metadata?.image_prompt, "Facebook");
...
async function publishLinkedInPersonal(client, row) {
  ...
  const text = withGopuHashtag(selectedCaption(row));
  if (!text.trim()) {
...
async function executeOnePublish(client, row, { dryRun = false } = {}) {
  ...
  const reservedImagePrompt = reserved.image_prompt || reserved.metadata?.image_prompt;
  if (platform === "LinkedIn" && !reserved.image_url && reservedImagePrompt) {
    const imgUrl = await generatePostImage(reservedImagePrompt, "LinkedIn");
    if (imgUrl) {
...
export async function runCmoPublishingEngine(options = {}) {
  ...
  const baseSelect = "id,tenant_id,run_id,platform,caption,generated_text,final_text,final_approved_content,hashtags,image_prompt,poster_url,image_url,approval_status,publish_status,live_post_url,post_url,publish_attempt_count,last_publish_attempt_at,last_publish_error,metadata,updated_at";
  let query = client
    .from("content_history")
    .select(baseSelect)
    .eq("approval_status", "approved")
