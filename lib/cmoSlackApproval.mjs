...
export function buildCmoSlackApprovalBlocks(row, options = {}) {
  ...
  const imageUrl = existingImageUrl(row);
  const imageGeneration = metadata(row).chatgpt_image_generation || metadata(row).chatgpt_image || metadata(row).canva_generation || metadata(row).canva || {};
  ...
  const blocks = [
    ...
    {
      type: "section",
      text: { type: "mrkdwn", text: `*Action needed*\nReview this ${row.platform || "social"} AI-generated content. Approve, reject, or request a modification before anything can publish.` }
    },
    {
      type: "section",
      fields: [
  ...
  if (imageUrl) {
    blocks.push({
      type: "image",
      image_url: imageUrl,
      alt_text: "GOPU OS CMO ChatGPT-generated creative preview"
    });
  } else {
    blocks.push({
      type: "section",
      text: {
        type: "mrkdwn",
        text: `*Image generation status*\n${cleanSlackText(truncate(imageGeneration.message || imageGeneration.status || "ChatGPT image generation is required before this can publish.", 1200))}`
      }
    });
  }
