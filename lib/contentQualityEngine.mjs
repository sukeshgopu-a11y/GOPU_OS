...
import { ensureCmoCanvaApprovalRow, ensureCmoCanvaDesignForApproval } from "./cmoCanvaWorkflow.mjs";
...
CANVA CREATIVE RULE:
For every CMO social post, AI generates only headline, slide content, caption, and hashtags. Canva brand templates render all graphics, logo, website, LinkedIn page name, colors, and visible text. Never create infographic text directly inside AI-generated images.
...
function generationPrompt({ platform, topic, tone, references = {} }) {
  ...
  if (isLinkedInPlatform(platform)) {
    guide = {
      ...
      image_style: "Canva-only 1080x1350 professional carousel/infographic template. Canva renders all visible text using template fonts and GOPU brand colors.",
      example_posts: [
  ...
  return JSON.stringify({
    ...
    output_schema: {
      headline: "short Canva-ready headline, maximum 12 words",
      slides: [
        {
          heading: "short Canva slide or section heading",
      ...
      canva_design_rule: "AI must not generate image text or infographic images. Canva templates render the final graphics, logo, website, LinkedIn page name, colors, and all text.",
...
async function persistGeneratedContent({ client, tenantId, platform, generated, model, mode }) {
  ...
  const contentResult = await client.from("content_history").insert({
    ...
    metadata: {
      ...
      canva_content: {
        ...
        hashtags: generated.hashtags || [],
        template_type: generated.canva_template_type || ""
      },
      image_prompt: "",
      hashtags: generated.hashtags,
      cta: generated.cta,
  ...
  if (contentResult.error) storage.errors.push(`content_history: ${contentResult.error.message}`);
    ...
    const canva = await ensureCmoCanvaDesignForApproval(contentHistory, { client });
    if (canva.ok) {
      storage.canva = true;
      contentHistory = canva.content_history || contentHistory;
    } else {
      storage.canva = false;
      storage.errors.push(`canva: ${canva.message || canva.status}`);
      contentHistory = canva.content_history || contentHistory;
    }
  }
