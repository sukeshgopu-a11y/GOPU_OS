...
function generationPrompt({ platform, topic, tone }) {
  ...
  return JSON.stringify({
    ...
    output_schema: {
      ...
      image_prompt: `Choose ONE of these research-verified templates that best matches the post topic, then write the full gpt-image-1 prompt: ${guide.image_style}. Rules: photorealistic render, professional product photography quality (match Everest Spices or Badshah Masala standard), ultra-sharp product detail, correct studio lighting for chosen style, no stock photo look, no watermarks. Include exact hex color codes for backgrounds where applicable. Describe text overlays with font weight and color if the template uses them. Output must be ready to paste directly into gpt-image-1.`,
