...
import { getChatGPTImageConnectionStatus } from "./cmoChatGPTImageWorkflow.mjs";
...
export async function getCreativeEngineStatus({ force = false } = {}) {
  ...
  const [openaiCreative, sharp, chatgptImage] = await Promise.all([
    checkOpenAICreativeProvider(),
    checkSharpPipeline(),
    getChatGPTImageConnectionStatus()
  ]);

  const providers = {
    openai_creative: openaiCreative,
    sharp,
    chatgpt_image: chatgptImage
  };
  const imageGenLive = chatgptImage.status === "live" || chatgptImage.status === "configured";
  const imageGenPending = openaiCreative.status === "pending";
  const status = imageGenLive ? "live" : imageGenPending ? "pending" : "error";
  const lastSuccessAt = imageGenLive ? new Date().toISOString() : null;
  ...
  cachedStatus = {
    ...
    render_pipeline: {
      poster_generation: imageGenLive,
      logo_stamping: false,
      chatgpt_image_generation: imageGenLive,
      ai_image_text_disabled: false,
      video_generation: false,
      video_generation_required_for_step_3: false
    },
    error_message: status === "live" ? null : firstProviderError({ chatgpt_image: chatgptImage, openaiCreative, sharp })
  };
  cachedAt = now;
  return cachedStatus;
}
