...
export async function getCreativeEngineStatus({ force = false } = {}) {
  ...
  const [openaiCreative, sharp] = await Promise.all([
  ...
  const posterComposerLive = openaiCreative.status === "live" && sharp.status === "live";
  ...
  cachedStatus = {
    ...
    render_pipeline: {
      poster_generation: openaiCreative.status === "live",
      logo_stamping: sharp.status === "live",
      video_generation: false,
      video_generation_required_for_step_3: false
    },
    error_message: status === "live" ? null : firstProviderError({ openaiCreative, sharp })
  };
  cachedAt = now;
  return cachedStatus;
}
