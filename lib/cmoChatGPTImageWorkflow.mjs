...
function buildImagePrompt(row = {}, contentType = "knowledge_carousel") {
  ...
  const base = [
    "Professional B2B social media post graphic for GOPU Exports, a premium Indian agricultural spice and rice export company.",
    "Deep navy blue (#0D2A4A) background with gold (#D4AF37) accent lines and white text areas.",
    "Clean modern layout. No human faces. No competitor logos. No visible pricing claims. No watermarks.",
    "High quality, photorealistic and design-forward, suitable for LinkedIn, Instagram, or Facebook marketing."
  ].join(" ");

  const typeDetails = {
    knowledge_carousel: "Knowledge-style infographic layout: numbered key points, clean icon row, subtle global trade route map in background.",
    shipment_announcement: "Shipment milestone visual: aerial container port photography, global shipping route overlay, premium branded packaging imagery.",
    market_update: "Market price update: commodity chart/trend graphic, spice imagery (chilli, turmeric, cumin), data-driven analytical feel.",
    product_spotlight: "Product spotlight: macro photography of premium Indian spices arranged on dark surface, laboratory-quality presentation, FSSAI/HACCP badge elements.",
    buyer_education: "Buyer education guide: checklist or numbered steps layout, compliance certification icons, professional business document visual."
  };

  return [
    base,
    typeDetails[contentType] || typeDetails.knowledge_carousel,
    `Post topic: ${headline}.`,
    topic && topic !== headline ? `Context: ${topic}.` : "",
    "Portrait 2:3 format. Brand: GOPU Exports - Global Spice & Rice Exporter. No fake UI screenshots, no mocked social platform frames."
  ].filter(Boolean).join(" ");
}
