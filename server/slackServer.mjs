...
const server = http.createServer((request, response) => {
  ...
  if (request.method === 'GET' && routePath === '/api/cto/integrations/status') {
    handleApiRoute(handleCtoIntegrationsApiStatus, request, response);
    return;
  }
  if (request.method === 'GET' && routePath === '/api/cto/provider-env/status') {
    handleApiRoute(handleCtoProviderEnvApiStatus, request, response);
    return;
  }
  if (request.method === 'POST' && routePath === '/api/cto/provider-env/save') {
    handleApiRoute(handleCtoProviderEnvApiSave, request, response);
    return;
  }
