...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    ...
    res.status(200).json({ ok: false, status: 'method_not_allowed', message: 'POST required.' });
    return;
  }
  ...
  if (typeof payload === 'string') {
    try {
      ...
      res.status(200).json({ ok: false, status: 'invalid_payload', message: 'Invalid publishing payload.' });
      return;
    }
  }

  try {
    const forcedDryRun = process.env.DRY_RUN?.trim() === 'true';
    const result = await runCmoPublishingEngine({
      dryRun: forcedDryRun || payload.dry_run === true || payload.dryRun === true,
      limit: payload.limit,
